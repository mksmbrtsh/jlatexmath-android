...
public class DefaultTeXFontParser {
	...
	public FontInfo[] parseFontDescriptions(FontInfo[] fi)
		...
		if (fontDescriptions != null) { // element present
			...
			for (int i = 0; i < list.getLength(); i++) {
				...
				InputStream is = null;
				if (base == null) {
					fi = parseFontDescriptions(fi, is = jLatexMath
	...
	protected void parseExtraPath() throws ResourceParseException, IOException {
		...
		if (syms != null) { // element present
			...
			SymbolAtom.addSymbolAtom(
					jLatexMath.getAssetManager().open(include), include);
		}
	...
	public static Typeface createFont(String name)
			throws ResourceParseException {
		Typeface f = Typeface.createFromAsset(jLatexMath.getAssetManager(),
				"fonts/" + name);
		return f;
			...
			for (int i = 0; i < list.getLength(); i++) {
				...
				try {
					...
					if (base == null) {
						map = factory
								.newDocumentBuilder()
								.parse(is = jLatexMath.getAssetManager().open(
